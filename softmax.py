# ---------------------------------------------------------------------------------------------#
#                                                                                              #
#               Functions for Modeling - Models, Training and Accuracy                         #
#                                                                                              #
# ---------------------------------------------------------------------------------------------#
...
def compute_probabilities(X, theta, temp_parameter):
    ...
    r = theta.dot(X.T) / temp_parameter # (matrix)
    c = np.max(r, axis = 0) # Fixed deduction factor for numerical stability  (vector)
...
def compute_kernel_probabilities(kernel_matrix, theta, temp_parameter):
    """
    Computes, for each datapoint X[i], the probability that X[i] is labeled as j
    for j = 0, 1, ..., k-1
    
    Args:
        kernel_matrix - (n, n) NumPy array - similarity matrix, each column: phi(x_1).phi(x_1) to phi(x_n).phi(x_1)
        theta - (k, n) NumPy array where row j represents the parameters of the model for label j
        temp_parameter - the temperature parameter of softmax function (scalar)
    ...
    r = theta.dot(kernel_matrix) / temp_parameter    # (matrix)
    c = np.max(r, axis = 0) # Fixed deduction factor for numerical stability (vector)
    H = np.exp(r - c)   # Subtracting the fixed amount c from each exponent to keep the resulting number from getting too large (matrix)
    H = H/np.sum(H, axis = 0)   # Dividing with the normalizing term
...
def compute_kernel_cost_function(kernel_matrix, Y, theta, lambda_factor, temp_parameter):
    """
    Computes the total cost over every datapoint.
    ...
    k = theta.shape[0]   # Number of categories
    n = kernel_matrix.shape[0]  # Numer of samples
    
    probabilities = np.clip(compute_kernel_probabilities(kernel_matrix, theta, temp_parameter), 1e-15, 1-1e-15) # Clip to stay inside the boundaries
    log_probabilities = np.log(probabilities)  # Log of probabilities
...
def run_kernel_gradient_descent(kernel_matrix, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # Using sparse matrices to handle large matrices efficiently (NumPy array of 1s and 0s)
    M = sparse.coo_matrix(([1]*n, (Y, range(n))), shape=(k,n)).toarray()    # Sparse matrix of [[y^(i)==j]] ; M[i][j] = 1 if y^(j) = 1 and 0 otherwise
    probabilities = compute_kernel_probabilities(kernel_matrix, theta, temp_parameter)   # Probabilities
    
    gradients = (-1 / (temp_parameter*n))* ((M - probabilities) @ kernel_matrix) + lambda_factor * theta # Calculating gradients
...
def softmax_kernel_regression(kernel_matrix, Y, temp_parameter, alpha, lambda_factor, k, num_iterations):
    """
    Runs batch gradient descent for a specified number of iterations on a dataset
    with theta initialized to the all-zeros array. Here, theta is a k by n NumPy array
    where row j represents the alpha values of our model for label j for j = 0, 1, ..., k-1
    ...
    Returns:
        theta - (k, n) NumPy array that is the final value of theta
        cost_function_progression - a Python list containing the cost calculated at each step of gradient descent
    """
 
    theta = np.zeros([k, len(Y)])
    cost_function_progression = []
    
    for i in range(num_iterations):
        cost_function_progression.append(compute_kernel_cost_function(kernel_matrix, Y, theta, lambda_factor, temp_parameter))
        theta = run_kernel_gradient_descent_iteration(kernel_matrix, Y, theta, alpha, lambda_factor, temp_parameter)
...
def get_kernel_classification(kernel_matrix, theta, temp_parameter):
    """
    Makes predictions by classifying a given dataset
    
    Args:
        kernel_matrix - (n, n) NumPy array - similarity matrix, each column: phi(x_1).phi(x_1) to phi(x_n).phi(x_1),
        test set kernel_matrix is (n,m) where m is the number of examples in the test set
        theta - (k, n) NumPy array where row j represents the parameters of the model for label j
        temp_parameter - the temperature parameter of softmax function (scalar)
    ...
    probabilities = compute_kernel_probabilities(kernel_matrix, theta, temp_parameter)
...
def compute_kernel_test_error(kernel_matrix, Y, theta, temp_parameter):
  """
  Returns the error of the labels when the classifier the classifier predicts the digit
  ...
  """  
    error_counts = 0.
    assigned_labels = get_kernel_classification(kernel_matrix, theta, temp_parameter)
